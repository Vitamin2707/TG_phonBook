...
import config
import create, check, search, delete, modify, import_export, db_link
from logger import *
...
def phonebook_option(update, _):
    ...
    if pb_option == '3':
        all_contacts = search.read_json(dbFilename)
...
def search_contact(update, _):
    ...
    if check.check_directory():                                     #Проверка не пустой ли справочник 
        # Считавание всей базы из файла 'db_directory.json'
        all_contacts = search.read_json(dbFilename)
...
def choose_contact(update, _):
    ...
    try:
        ...
        if contact_index > 0:
            ...
            # Пишем в журнал
            logger.info("Комментарий %s: %s", user.first_name,
